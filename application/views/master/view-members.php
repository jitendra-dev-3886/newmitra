										    ...
										    <?php if(is_array($members)){foreach($members as $rec){ ?>
    											<tr>
    												<td style="width:200px">
    												    ...
    												    <span class="text-info"><?php $sql3 = "select * from exr_package where package_id='".$rec->package_id."'";
                                                        	$checkst2 = $this->db->query($sql3)->result_array();                                             
                                                            $pack = $checkst2[0]['package_name'];
                                                            $pk= "Package: ";
                                                            if($checkst2) echo $pk.$pack; ?></span><br>
                                                        <span class="text-warning"><?php $sql = "select * from dmt_commission_slab where dmt_comm_id='".$rec->dmt_comm_id."'";
                                                        	$checkst = $this->db->query($sql)->result_array();                                             
                                                            $dmt_pack = $checkst[0]['slab'];
                                                            $dmt_comm_id = $checkst[0]['dmt_comm_id'];
                                                            $dmt_pk= "DMT: ";
                                                            if($checkst) echo $dmt_pk.$dmt_pack.'('.$dmt_comm_id.')'; ?></span>
    												</td>
    												...
    												<td style=" width: 200px;"><b>
    												    <a href="<?php echo base_url(); ?>master/member-details/<?php echo $rec->cus_id; ?>"><i class="typcn typcn-eye text-info"></i></a>
    												    <a href="<?php echo base_url(); ?>master/edit-member/<?php echo $rec->cus_id; ?>"><i class="typcn typcn-edit text-success"></i></a>
    												   <!-- <i class="typcn typcn-arrow-repeat text-primary"></i>-->
    												    <a href="<?php echo base_url();?>master/delete-account/<?php echo $rec->cus_id; ?>" onclick="return confirm('Are you sure You want to Delete User Id <?php echo $rec->cus_id; ?>?')" ><i class="typcn typcn-trash text-danger"></i></a> 
    												    <a href="<?php echo base_url()?>master/switch-account/<?php echo $rec->cus_id; ?>"><i class="typcn typcn-weather-snow text-warning"></i></a>
    												    <a href="<?php echo base_url(); ?>master/logout-member/<?php echo $rec->cus_id; ?>" onclick="return confirm('Are you sure You want to Logout This User?')" > <i class="typcn typcn-power-outline text-danger"></i> </a>
    												</b></td>
