...
class wpl_global
{
    ...
    public static function get_property_types($property_type_id = 0, $enabled = 1)
    {
        if (!is_numeric($property_type_id) || (int) $property_type_id <= 0) {
            ...
            if (isset(self::$property_types[$enabled])) {
                return self::$property_types[$enabled];
            }
            ...
            // Return from cache if exists
            if (isset(self::$property_types_by_id[$property_type_id])) {
                return self::$property_types_by_id[$property_type_id];
            }
            $query = "SELECT * FROM `#__wpl_property_types` ORDER BY `index` ASC";
            $results = wpl_db::select($query, 'loadObjectList');
            // Add to Cache
            foreach ($results as $result) {
                self::$property_types_by_id[$result->id] = $result;
            }
    ...
    public static function get_listings($listing_id = 0, $enabled = 1)
    {
        if (!is_numeric($listing_id) || (int) $listing_id <= 0) {
            // Return from cache if exists
            if (isset(self::$listing_types[$enabled])) {
                return self::$listing_types[$enabled];
            }
            ...
            // Return from cache if exists
            if (isset(self::$listing_types_by_id[$listing_id])) {
                return self::$listing_types_by_id[$listing_id];
            }
            $query = "SELECT * FROM `#__wpl_listing_types` ORDER BY `index` ASC";
            $results = wpl_db::select($query, 'loadObjectList');
            /** add to cache **/
            foreach ($results as $result) {
                self::$listing_types_by_id[$result->id] = $result;
            }
    ...
    public static function get_full_url()
    {
        ...
        $page_url = 'http';
        if (isset($server['HTTPS']) and $server['HTTPS'] == 'on' or is_ssl()) {
            $page_url .= 's';
        }
    ...
    public static function order_table($thName, $orderBy, $class = true, $url = '')
    {
        ...
        if ($qs_orderby == $orderBy and $class == true) {
            $class = $orderType == 'ASC' ? 'class="desc"' : 'class="asc"';
        }
    ...
    public static function svg($image, $id = NULL, $params = array(), $path = '')
    {
        ...
        if (!trim($path)) {
            $path = wpl_global::get_wpl_root_path() . 'assets' . DS . 'img' . DS;
        }
        ...
        foreach ($params as $key => $value) {
            if ($value and strpos($tag, $key . '="') !== false) {
                $tag = preg_replace('/' . $key . '="(.*?)"/', $key . '="' . $value . '"', $tag);
            } elseif ($value) {
                $tag = str_replace('<svg', '<svg ' . $key . '="' . $value . '"', $tag);
            }
        }
    ...
    public static function load_js_template($template, $path = '')
    {
        ...
        /** set default path **/
        if (!trim($path)) {
            $path = wpl_global::get_wpl_root_path() . 'assets' . DS . 'js' . DS . 'js_templates.tmpl';
        }
    ...
    public static function get_wp_url($type = 'site')
    {
        ...
        if (in_array($type, array('frontend', 'site'))) {
            $url = rtrim(home_url(), '/') . '/';
        } elseif (in_array($type, array('backend', 'admin'))) {
            $url = admin_url();
        } elseif ($type == 'wordpress') {
            $url = rtrim(get_site_url(), '/') . '/';
        } elseif ($type == 'wpl') {
            $url = rtrim(plugins_url(), '/') . '/' . WPL_BASENAME . '/';
        } elseif ($type == 'upload') {
            $url = rtrim(get_site_url(), '/') . '/wp-content/uploads/WPL/';
        } elseif ($type == 'content') {
            $url = rtrim(content_url(), '/') . '/';
        } elseif ($type == 'plugin') {
            $url = rtrim(plugins_url(), '/') . '/';
        } elseif ($type == 'include') {
            $url = includes_url();
        } else {
            $url = false;
    ...
    {
        if (isset($a['sort']) and isset($b['sort']) and $a['sort'] > $b['sort']) {
            return 1;
        } elseif (isset($a['sort']) and isset($b['sort']) and $a['sort'] < $b['sort']) {
            return -1;
        } elseif (isset($a['sort']) and isset($b['sort']) and $a['sort'] == $b['sort']) {
            return 0;
        }
    ...
    {
        if (trim($addon_id)) {
            return wpl_db::get('*', 'wpl_addons', 'id', $addon_id, false);
        } else {
            return wpl_db::get('*', 'wpl_addons', 'addon_name', $addon_name, false);
        }
    }
    /**
    ...
    public static function check_addon_update($addon_id)
    {
        ...
        if (!$addon_data->updatable) {
            return array('success' => 0, 'message' => __('Error: #U201, An add-on update is not available for this domain or update key!', 'real-estate-listing-realtyna-wpl'));
        }
        $phpver = phpversion();
        ...
        foreach ($raw_addons as $raw_addon) {
            $addons[$raw_addon['id']] = array('name' => $raw_addon['name'], 'addon_name' => $raw_addon['addon_name'], 'version' => $raw_addon['version'], 'id' => $raw_addon['id']);
        }
        ...
        $POST = array('domain' => $domain, 'id' => $addon_id, 'is_addon' => '1', 'wpversion' => $wpversion, 'wplversion' => $wplversion, 'version' => $version, 'phpver' => $phpver, 'theme_name' => $theme_name, 'theme_version' => $theme_version, 'addons' => $addons, 'admin_email' => get_option('admin_email'), 'update_key' => $update_key, 'support_key' => $support_key, 'username' => urlencode($username), 'password' => urlencode($password), 'command' => 'check_update', 'format' => 'json');
    ...
    public static function check_all_update()
    {
        ...
        foreach ($addons as $addon) {
            self::check_addon_update($addon['id']);
        }
    }
    /**
    ...
    public static function get_current_language()
    {
        ...
        if ($wpllang = wpl_request::getVar('wpllang', NULL)) {
            return $wpllang;
        }
    ...
    public static function get_upload_base_path($blog_id = NULL)
    {
        ...
        if (!$blog_id or $blog_id == 1) {
            if (!wpl_folder::exists($ABSPATH)) {
                wpl_folder::create($ABSPATH);
            }
            return $ABSPATH;
        } else {
    ...
    public static function get_upload_base_url($blog_id = NULL)
    {
        ...
        $ABSPATH = wpl_global::get_upload_abs('path');
        if (!$blog_id or $blog_id == 1) {
            return wpl_global::get_upload_abs('url') . 'WPL/';
        } else {
            $path = rtrim($ABSPATH, DS) . $blog_id . DS;
            if (!wpl_folder::exists($path)) {
                wpl_folder::create($path);
    ...
    public static function get_wpl_item_links($exclude = array(), $start = 0, $offset = 2000)
    {
        ...
        /** exclude **/
        $criteria = NULL;
        if (isset($exclude['properties']) and is_array($exclude['properties']) and count($exclude['properties'])) {
            $criteria = " AND `mls_id` NOT IN (" . implode(',', $exclude['properties']) . ")";
        }
        /** limit **/
        $limit = $start . ', ' . $offset;
        if ($offset == -1) {
        ...
        if ($check_multilingual_status) {
            wpl_request::setVar('wpllang', $current_language);
        }
    ...
    public static function get_layouts($view = 'property_listing', $exclude = array('message.php'), $client = 'frontend')
    {
        ...
        foreach ($files as $file) {
            if (in_array($file, $exclude) or strpos($file, '_k') !== false or strpos($file, 'internal_') !== false) {
                continue;
            }
    ...
    public static function get_listing_types_by_parent($parent, $enabled = 1)
    {
        ...
        // Return from cache if exists
        if (isset(self::$listing_types_by_parent[$key])) {
            return self::$listing_types_by_parent[$key];
        }
    ...
    public static function get_property_types_by_parent($parent, $enabled = 1)
    {
        ...
        $key = $parent . '_' . $enabled;
        // Return from cache if exists
        if (isset(self::$property_types_by_parent[$key])) {
            return self::$property_types_by_parent[$key];
        }
    ...
    public static function switch_language($language)
    {
        ...
        if ($wpl or $real_estate_listing_realtyna_wpl) {
            wpl_request::setVar('wpllang', $language);
        }
    ...
    public static function get_language_mo_path($locale = NULL, $textdomain = 'real-estate-listing-realtyna-wpl')
    {
        ...
        if ($textdomain == 'wpl') {
            ...
            if (!wpl_file::exists($path)) {
                $path = wpl_global::get_wpl_root_path() . 'languages' . DS . 'wpl' . '-' . $locale . '.mo';
            }
        } else {
            $path = WP_LANG_DIR . DS . 'plugins' . DS . 'real-estate-listing-realtyna-wpl' . '-' . $locale . '.mo';
            if (!wpl_file::exists($path)) {
                $path = wpl_global::get_wpl_root_path() . 'languages' . DS . 'real-estate-listing-realtyna-wpl' . '-' . $locale . '.mo';
            }
        }
        return $path;
    }
    ...
    public static function generate_request_str($vars = array())
    {
        ...
        foreach ($vars as $field => $value) {
            if (!is_string($value) or is_string($value) and trim($value) == '') {
                continue;
            }
    ...
    public static function generate_pages_selectbox($selected)
    {
        ...
        foreach ($pages as $page) {
            $output .= '<option ';
            if (isset($selected) and $page->ID == $selected) {
    ...
    public static function get_realtyna_announcements()
    {
        ...
        $POST = array('command' => 'get_announcements', 'basic' => $basic, 'version' => wpl_global::wpl_version(), 'format' => 'json');
        $io_handler = 'https://billing.realtyna.com/io/io.php';
        $result = wpl_global::get_web_page($io_handler, $POST);
        $answer = json_decode($result, true);
        $status = isset($answer['status']) ? $answer['status'] : 0;
    ...
    public static function wpl_minimize_price($price)
    {
        if ($price < 1000) {
            return round($price);
        } elseif ($price >= 1000 and $price < 100000) {
            return round($price / 1000, 1) . 'K';
        } elseif ($price >= 100000 and $price < 1000000) {
    ...
    public static function render_pattern($the_pattern, $property_id, $property_data, $glue = ',', $data = NULL)
    {
        ...
        if (is_null($data)) {
            ...
            if (trim($property_data['location1_name'])) {
                $data['location1'] = __($property_data['location1_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location2_name'])) {
                $data['location2'] = __($property_data['location2_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location3_name'])) {
                $data['location3'] = __($property_data['location3_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location4_name'])) {
                $data['location4'] = __($property_data['location4_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location5_name'])) {
                $data['location5'] = __($property_data['location5_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location6_name'])) {
                $data['location6'] = __($property_data['location6_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['location7_name'])) {
                $data['location7'] = __($property_data['location7_name'], 'real-estate-listing-realtyna-wpl');
            }
            if (trim($property_data['zip_name'])) {
                $data['zipcode'] = __($property_data['zip_name'], 'real-estate-listing-realtyna-wpl');
            }
            // Location Abbr Names
            if (isset($property_data['location1_name']) and trim($property_data['location1_name'])) {
                $data['location1_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location1_name'], 1), 'real-estate-listing-realtyna-wpl');
            }
            if (isset($property_data['location2_name']) and trim($property_data['location2_name'])) {
                $data['location2_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location2_name'], 2), 'real-estate-listing-realtyna-wpl');
            }
        ...
        foreach ($matches_pattern[1] as $pattern) {
            ...
            if (strpos($pattern, 'str:') !== false) {
                ...
                if ($condition and (isset($data[$condition]) and trim($data[$condition]) or isset($property_data[$condition]) and trim($property_data[$condition]))) {
                    $rendered = str_replace('[' . $pattern . ']', __($text, 'real-estate-listing-realtyna-wpl'), $rendered);
                } elseif ($condition) {
                ...
                if (wpl_global::check_multilingual_status() and wpl_addon_pro::get_multiligual_status_by_column($pattern, isset($property_data['kind']) ? $property_data['kind'] : 2)) {
                    $pattern_multilingual = wpl_addon_pro::get_column_lang_name($pattern, wpl_global::get_current_language(), false);
                }
                ...
                if ($rendered_value) {
                    if (isset($rendered_value['values'])) {
                        $rendered = str_replace('[' . $pattern . ']', implode(' ', $rendered_value['values']), $rendered);
                    } else {
                        $rendered = str_replace('[' . $pattern . ']', $rendered_value['value'], $rendered);
                    }
                } elseif (trim($value)) {
                    $rendered = str_replace('[' . $pattern . ']', $value, $rendered);
                } else {
                    $rendered = str_replace(' [' . $pattern . ']', '', $rendered);
                    $rendered = str_replace('[' . $pattern . ']', '', $rendered);
                }
    ...
    public static function include_google_recaptcha($option, $property_id = NULL)
    {
        ...
        if ($gre_section === '1') {
            return '<div ' . $id_attr . ' class="g-recaptcha" data-sitekey="' . esc_attr__($g_site_key) . '"></div>' . $script;
        }
        return true;
    }
    /**
    ...
    public static function verify_google_recaptcha($response, $option)
    {
        ...
        if ($enabled === '1' && $gre_section === '1') {
            ...
            } elseif (!wpl_global::get_recaptcha_response($response)) {
                return array('success' => 0, 'message' => __('Captcha is invalid! Please try again.', 'real-estate-listing-realtyna-wpl'));
            } else {
    ...
    public static function get_recaptcha_response($response, $remote_ip = NULL)
    {
        ...
        if (is_null($remote_ip)) {
            $remote_ip = wpl_users::get_current_ip();
        }
        ...
        if (isset($reply['success']) and trim($reply['success'])) {
            return true;
        } else {
            return false;
        }
    ...
    {
        if ($search_fields == NULL or !$search_fields) {
            return false;
        }
        ...
        foreach ($search_fields as $field => $value) {
            if (strpos($field, $needle_str) === false) {
                continue;
            }
            $ex = explode('_', $field);
            $format = $ex[1];
            if (count($ex) == 2) {
                $table_column = str_replace($needle_str, '', $field);
            } else {
    ...
    public static function generate_readable_criteria_values($table_column, $value)
    {
        ...
        $dbst_id = wpl_flex::get_dbst_id($table_column);
        if ($dbst_id) {
            $field_values = wpl_flex::get_field_values($dbst_id);
        }
        if ($field_values and isset($field_values[$value])) {
            $generated_value = $field_values[$value]['label'];
        } else {
    ...
    public static function tags_add_indexes()
    {
        ...
        foreach ($kinds as $kind) {
            ...
            foreach ($tags as $tag) {
                wpl_db::index_add($tag->table_column, $tag->table_name);
            }
        }
    }
    /**
    ...
    public static function is_bot()
    {
        $bot_names = array(
            ...
            // other bots
            array('IBM', 'Almaden', 'active'),
            //IBM research project
            array('Cyveillance', 'Cyveillance', 'active'),
            //A snoop bot checking for copyright/trademark violations
            array('DTSearch', 'DTSearch', 'active'),
            //Retail search software
            array('Girafa.com', 'Girafa', 'active'),
            //Client program
            array('Indy Library', 'IndyLibrary', 'active'),
            //Email harvester
            array('LinkWalker', 'LinkWalker', 'active'),
            //Link verification
            array('MarkWatch', 'MarkWatch', 'active'),
            //A snoop bot checking for trademark violations
            array('NameProtect', 'NameProtect.com', 'active'),
            //A snoop bot checking for trademark violations
            array('Robozilla', 'DMOZ', 'active'),
            //Link checker used by DMOZ
            array('Teradex Mapper', 'Teradex directory', '-'),
            array('Tracerlock', 'Tracerlock.com', 'active'),
            //News monitoring service
            array('W3C_Validator', 'W3C', 'active'),
            //HTML Validator Added 5.2003 by Mark Parnell of www.clarkecomputers.com.au
            array('WDG_Validator', 'WDG', 'active'),
            //HTML Validator Added 5.2003 by Mark Parnell of www.clarkecomputers.com.au
            array('Zealbot', 'Looksmart', 'active'),
        );
        ...
        $regex_string .= '/i';
        return (bool) (isset($_SERVER['HTTP_USER_AGENT']) && preg_match($regex_string, $_SERVER['HTTP_USER_AGENT']));
    }
    /**
    ...
    public static function remove_duplicated_indexes()
    {
        ...
        foreach (array('wpl_properties', 'wpl_items') as $table) {
            ...
            // No Index
            if (!is_array($indexes) or is_array($indexes) and !count($indexes)) {
                continue;
            }
            // Unique Keys
    ...
    {
        if ($format == 12) {
            $formatted = date('g:i A', $hour * 3600);
        } else {
            $formatted = date('G:i', $hour * 3600);
        }
        return $formatted;
    }
    /**
    ...
    {
        wp_add_inline_script('wp-widgets', file_get_contents('assets/js/widget_block_editor.js', true));
    }
    /**
